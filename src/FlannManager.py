...
class FlannManager (object):
    ...
    def __get_key_points(self, image_path):
        ...
        while (do_surf):
            ...
            (keypoints, descriptors) = cv.ExtractSURF(
                image, None, cv.CreateMemStorage(), (1, range, 3, 1))
            do_surf = False
    ...
    def add_photo(self, id, path):
        ...
        try:
            ...
            f = file(self.flann_path +
                     self.flann_descriptors + image_name + ".keypoints", "w")
            pickle.dump(descriptors, f)
            f.close()

            print "* Photo %s with %d keypoints has been added" % (
                image_name, len(descriptors))
    ...
    def create_index(self):
        ...
        try:
            ...
            key_point_array = require(
                self.key_point_list, float32, self.default_flags)
            ...
            self.myflann = FLANN()
            params = self.myflann.build_index(
                result, target_precision=0.9, log_level="info")
            ...
            for fdescriptors in ls:
                ...
                f = file(self.flann_path +
                         self.flann_descriptors + fdescriptors, "r")
            ...
            self.__lemur_create_index(
                self.flann_path + "vwords", self.lemur_path + "index")
    ...
    def __lemur_create_index(self, path_file_list, path_dest_index):
        ...
        builder_xml = f.read()
        builder_xml = (builder_xml.replace("$PATH_FILE_LIST", path_file_list)
                       ).replace("$PATH_DEST_INDEX", path_dest_index)
        f.close()
    ...
    def __lemur_create_query(self, path_index_key, path_query, path_result, number_results):
        ...
        query_xml = f.read()
        query_xml = (query_xml.replace("$PATH_INDEX_KEY",
                     path_index_key)).replace("$PATH_QUERY", path_query)
        query_xml = (query_xml.replace("$PATH_RESULT", path_result)
                     ).replace("$NUMBER_RESULTS", number_results)
        f.close()
    ...
    def query(self, image_path):
        ...
        self.__lemur_create_query(self.lemur_path + "index",
                                  sometext, self.lemur_path + "result", "10")
        try:
            ...
            p = subprocess.Popen(
                [self.lemur_bin_path + "IndriRunQuery",
                    self.lemur_path + "query.xml"],
    ...
    def __get_visual_words(self, descriptors):
        ...
        flann.find_nearest_neighbors_index[key_point_array.dtype.type](
            self.myflann._FLANN__curindex,
